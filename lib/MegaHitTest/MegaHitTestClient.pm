package MegaHitTest::MegaHitTestClient;
...
sub new
{
    ...
    my $self = {
	client => MegaHitTest::MegaHitTestClient::RpcClient->new,
	url => $url,
	headers => [],
    };
...
<pre>
$params is a MegaHitTest.MegaHitParams
$output is a MegaHitTest.MegaHitOutput
...
# Authentication: required
	...
	method => "MegaHitTest.run_megahit",
	params => \@args,
    });
    if ($result) {
	if ($result->is_error) {
...
sub _validate_version {
    ...
    if ($sMinor > $cMinor) {
        warn "New client version available for MegaHitTest::MegaHitTestClient\n";
    }
    if ($sMajor == 0) {
        warn "MegaHitTest::MegaHitTestClient version is $svr_version. API subject to change.\n";
    }
}
...
package MegaHitTest::MegaHitTestClient::RpcClient;
