...
def get_config():
    ...
    for nameval in config.items(get_service_name() or 'MegaHitTest'):
        retconfig[nameval[0]] = nameval[1]
    return retconfig

config = get_config()

from MegaHitTest.MegaHitTestImpl import MegaHitTest
impl_MegaHitTest = MegaHitTest(config)
...
async_run_methods = {}
async_check_methods = {}
async_run_methods['MegaHitTest.run_megahit_async'] = ['MegaHitTest', 'run_megahit']
async_check_methods['MegaHitTest.run_megahit_check'] = ['MegaHitTest', 'run_megahit']
sync_methods['MegaHitTest.run_megahit'] = True
...
class Application(object):
    ...
    def __init__(self):
        submod = get_service_name() or 'MegaHitTest'
        ...
        self.rpc_service.add(impl_MegaHitTest.run_megahit,
                             name='MegaHitTest.run_megahit',
                             types=[dict])
        self.method_authentication['MegaHitTest.run_megahit'] = 'required'
                    ...
                    if auth_req != "none":
                        if token is None and auth_req == 'required':
                            ...
                            err.data = "Authentication required for " + \
                                "MegaHitTest but no authentication header was passed"
                            raise err
